...
[支持的管道类型：](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Pipeline_Configuration/ProductIcons_Doc.html#concept_mjg_ly5_pgb)![img](imgs/icon-SDC-20200310172509989.png) 资料收集器

Kinesis Consumer来源从Amazon Kinesis Streams读取数据。

配置Kinesis Consumer时，您可以为Kinesis集群指定Amazon Web Services连接信息以及源数据的数据格式。

Kinesis Consumer源可以使用多个线程来启用来自集群的数据的并行处理。

Kinesis Consumer原点将偏移量存储在DynamoDB租借表中。您可以选择定义标签以应用于由来源创建的租赁表。必要时，您可以重置Kinesis Consumer管道的原点，但是一定要在重审需求和准则之前。

您可以选择使用代理连接到Kinesis Streams。

## 多线程处理

Kinesis Consumer起源可以执行并行处理，并可以创建多线程管道。原点基于“最大线程数”属性使用多个并发线程。
...
当Data Collector从Kinesis Consumer来源读取数据时，它必须将凭证传递给Amazon Web Services。
...
您可以配置Kinesis Consumer的读取间隔。读取间隔确定了Kinesis Consumer要从Kinesis分片请求其他数据之前等待的时间。默认情况下，Kinesis Consumer在两次请求之间等待一秒钟。

调整读取间隔以适应Kinesis数据加载。例如，您可以减少等待时间以提高吞吐量，或者在数据流变慢时增加等待时间。

亚马逊建议读取间隔为1秒。Kinesis Consumer的最小读取间隔为200毫秒，这使得每秒可以轮询五次。要增加总容量，可以为集群创建其他分片。

## 租赁表标签

Kinesis Consumer原点将偏移量（原点停止读取的位置）存储在DynamoDB租借表中。您可以选择定义标签以应用于由来源创建的租赁表。源不能将标签添加到现有的租用表。

标签允许您按用途，所有者，环境或任何其他分类对DynamoDB资源进行分类。

每个Kinesis应用程序使用一个DynamoDB租约表。如果您有多个使用同一应用程序的Kinesis Consumer源，那么所有源都使用相同的DynamoDB租约表。如果源不存在，则源为该应用程序创建一个DynamoDB租用表。

源仅在创建租用表时才分配已配置的标签。如果该应用程序已经存在租用表，则源将使用现有表，并且不会将任何已配置的标签分配给现有表。

配置原点时，请在“租赁表”选项卡上定义标签。要添加标签，请单击添加并定义标签名称和值。DynamoDB标记名称和值区分大小写。

有关为DynamoDB进行标记的更多信息，请参阅[Amazon Web Services文档](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Tagging.html)。

## 重置Kinesis消费者来源

您可以重置Kinesis Consumer原点以处理所有可用数据，而不是从上次保存的偏移量继续。有关重置原点的一般信息，请参见[重置原点](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Pipeline_Maintenance/ResettingTheOrigin.html#task_hdg_j1s_5q)。

当您为Kinesis Consumer管道 重置原点时，Data Collector会为每个使用相同应用程序名称的Kinesis Consumer管道重置原点。

重置原点之前，请注意以下要求和准则：

- 要求：删除DynamoDB上的表权限

  因为Kinesis Streams在DynamoDB中存储偏移量，所以与管道关联的IAM用户或IAM角色必须具有与管道中使用的应用程序名称匹配的表的删除表权限，如下所示：如果您使用访问密钥向Amazon Web Services进行身份验证，则与访问密钥关联的IAM用户必须具有删除表权限。如果Data Collector使用实例配置文件进行身份验证，则EC2实例的IAM角色必须具有删除权限。

- 重置原点的准则

  要重置Kinesis Consumer管道的原点，请执行以下步骤：停止要重置的管道。如果其他管道使用相同的应用程序名称，请停止所有管道。重置原点：在管道画布中或从主页中，单击更多图标![img](imgs/icon_MonMore.png)，然后单击**重置原点**。当使用使用相同应用程序名称的多个管道时，可以使用任何管道。您只需要重置其中之一即可。用重置的原点启动管道。如果您停止了任何相关的管道，请重新启动这些管道。

例如，假设您有三个使用Sales应用程序名称从Kinesis Streams读取的管道：NorthSales，CentralSales和SouthSales。要重置所有管道的原点，请停止所有三个管道，并重置其中一条管道的原点，例如SouthSales。然后重新启动SouthSales管道，然后可以重新启动其他管道。

## 资料格式

Kinesis Consumer起源基于数据格式对数据进行不同的处理。Kinesis Consumer可以处理以下类型的数据：
...
配置Kinesis消费者来源以从Amazon Kinesis Streams读取数据。
...
2. 在**Kinesis**选项卡上，配置以下属性：
   ...
   | Kinesis属性                                                  |                                                              |
   | :----------------------------------------------------------- | :----------------------------------------------------------- |
   | [访问密钥ID](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Origins/KinConsumer.html#concept_oxv_t5g_vw) | AWS访问密钥ID。不将IAM角色与IAM实例配置文件凭据一起使用时是必需的。 |
   | [秘密访问密钥](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Origins/KinConsumer.html#concept_oxv_t5g_vw) | AWS秘密访问密钥。不将IAM角色与IAM实例配置文件凭据一起使用时是必需的。 |
   | 区域                                                         | 托管Kinesis集群的Amazon Web Services地区。                   |
   | 终点                                                         | 当您为区域选择“其他”时要连接的端点。输入端点名称。           |
   | 流名称                                                       | Kinesis流名称。                                              |
   | 应用名称                                                     | Kinesis应用程序名称。应用程序名称等效于Kafka用户组。         |
   | 初始位置                                                     | 确定当DynamoDB中先前没有存储应用程序名称的偏移量时，Data Collector从何处开始读取。使用以下选项之一：最新-开始读取在管道启动后到达的消息。TRIM_HORIZON-从流的开头开始读取消息。AT_TIMESTAMP-从指定的时间戳开始读取消息。 |
   | 初始时间戳                                                   | 从将初始位置设置为AT_TIMESTAMP时开始读取消息的时间起的毫秒数。 |
   | 最大批处理大小（消息）                                       | 批处理中包含的最大邮件数。接受的值最高为Data Collector的最大批处理大小。默认值是1000 数据采集器默认设置为1000。 |
   | [读取间隔（毫秒）](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Origins/KinConsumer.html#concept_y55_dz4_yr) | 从分片请求其他数据之前需要等待的毫秒数。亚马逊建议1,000。最小值为200。默认值为1,000。 |
   | [最大线程](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Origins/KinConsumer.html#concept_jhh_psb_mz) | 原始服务器生成并用于多线程处理的线程数。输入正整数或计算结果为正整数的表达式。默认情况下，源使用以下表达式根据Data Collector 计算机上可用的CPU内核数来配置属性：`${runtime:availableProcessors()}`启动管道时，将评估此表达式。 |

3. 在“ **租借表”**选项卡上，可以选择配置标记以应用于由来源创建的DynamoDB租借表。

   使用[简单或批量编辑模式](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Pipeline_Configuration/SimpleBulkEdit.html#concept_alb_b3y_cbb)，单击**添加**图标以添加其他标签。

   有关标签的更多信息，请参见[租赁表标签](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Origins/KinConsumer.html#concept_qy4_rky_vcb)。
...
4. 在“ **高级”**选项卡上，可以选择配置高级属性。
   ...
   | 油门等待时间     | 在Kinesis限制读取之后，要等待的毫秒数，然后再次尝试检查点。  |
   | 油门最大重试次数 | Kinesis限制读取后重试检查点的最大次数。                      |

5. 在“ **数据格式”**选项卡上，配置以下属性：

   | 数据格式属性                                                 | 描述                                                         |
   | :----------------------------------------------------------- | :----------------------------------------------------------- |
   | [资料格式](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Origins/KinConsumer.html#concept_cwg_r2m_35) | 要读取的数据类型。使用以下选项之一：阿夫罗二元定界JSON格式记录原虫[SDC记录](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Pipeline_Design/SDCRecordFormat.html#concept_qkk_mwk_br)文本XML格式 |

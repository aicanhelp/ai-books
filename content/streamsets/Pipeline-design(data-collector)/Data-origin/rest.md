...
[支持的管道类型：](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Pipeline_Configuration/ProductIcons_Doc.html#concept_mjg_ly5_pgb)![img](imgs/icon-SDC-20200310173709160.png) 资料收集器

REST服务源是一个多线程源，它在HTTP端点上侦听请求并处理所有授权的请求。当与同一管道中的一个或多个“向原始目的地发送响应”一起使用时，原始地址还可以将带有状态代码的记录发送回原始REST API客户端。

使用REST Service起源创建基于REST的[微服务](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Microservice/Microservice_Title.html#concept_qfh_xdm_p2b)。在微服务管道中，REST服务源与一个或多个[微服务目标一起使用](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Microservice/Microservice_Title.html#concept_wht_tbf_kfb)，这些[目标](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Microservice/Microservice_Title.html#concept_wht_tbf_kfb)指定HTTP响应代码以传递回源REST API客户端。

REST服务源为收到的每个请求生成一个批处理。REST服务源可以使用多个线程来启用对来自多个客户端的请求的并行处理。在配置源之前，请执行其他步骤以配置客户端。

配置REST Service来源时，可以定义侦听端口，应用程序ID和最大消息大小。您可以配置并发请求的最大数量，以确定要使用的线程数。您还可以配置SSL / TLS属性，包括默认的传输协议和密码套件。并且，您可能需要相互的SSL / TLS身份验证。您还可以配置生成的响应的格式。

REST服务源可以读取包含无压缩或Gzip或Snappy压缩格式的消息的请求。

## 先决条件

在使用REST Service来源运行管道之前，请完成以下先决条件才能配置REST API客户端。

### 在请求中包含应用程序ID

配置REST API客户端以在每个请求中包括REST服务应用程序ID。

在配置REST服务源时，您将定义一个应用程序ID，该ID用于将请求传递到源。发送到源的所有消息都必须包含应用程序ID。

通过以下方式之一包括每个客户端请求的应用程序ID：

- 在请求头中

  将以下信息添加到您希望源处理的所有请求的HTTP请求标头中：`X-SDC-APPLICATION-ID: `

  例如：`X-SDC-APPLICATION-ID: sdc_microservice`

- 在URL中的查询参数中

  如果无法配置客户端请求标头（例如，如果请求是由另一个系统生成的，则），请配置每个REST API客户端以将数据发送到在查询参数中包括应用程序ID的URL。

  要在查询参数中包括应用程序ID，请在配置源时**在URL**属性中启用 **应用程序ID**。然后，在URL查询参数中包含应用程序ID。

  URL使用以下格式：`://:/?sdcApplicationId=`

  该URL包括以下组件：<http | https>-使用https进行安全的HTTP连接。<sdc_hostname>- 数据收集器主机名。<listening_port>-源侦听数据的端口号。<application_ID>-为REST服务源定义的应用程序ID。

  例如： `https://localhost:8000/?sdcApplicationId=sdc_microservice`

### 将数据发送到侦听端口

配置REST API客户端以将数据发送到REST Service侦听端口。

配置起点时，您将定义一个监听端口号，起点将在此监听数据。要将数据传递到管道，请配置每个REST API客户端以将数据发送到包含侦听端口号的URL。
...
- <path>-可选。URL的路径。
- <queryString>-可选。路径后面的URL参数。
- <application_ID>-可选的[应用程序ID（](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Origins/RESTService.html#concept_ttw_wtd_kz)如果未在请求标头中传递）。

例如：`https://localhost:8000/`或 `https://localhost:8000/rest/v1/user`。

## 多线程处理

REST服务源执行并行处理，并允许创建多线程管道。

REST服务源基于“最大并发请求数”属性使用多个并发线程。启动管道时，源将创建在“最大并发请求数”属性中指定的线程数。每个线程都根据传入的请求生成一个批处理，并将该批处理传递给可用的管道运行器。
...
例如，假设您将“最大并行请求数”属性设置为5。启动管道时，源将创建五个线程，而Data Collector 会创建匹配数量的管道运行器。 接收到数据后，原点将批处理传递给每个管道运行器进行处理。 在批处理中，REST服务仅包含具有指定应用程序ID的REST API请求。
...
在[微服务管道中](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Microservice/Microservice_Title.html#concept_qfh_xdm_p2b)，REST服务源可以将响应发送回源REST API客户端。
...
- method-请求的方法，例如GET，POST或DELETE。
- path-URL的路径。
- queryString-路径后的URL参数。
- remoteHost-发出请求的客户端或代理的名称。

REST服务源还包括记录中的HTTP请求标头字段（例如Host或Content-Type）作为记录标头属性。属性名称与原始HTTP请求标头字段名称匹配。

## 资料格式

REST服务源根据您选择的数据格式对数据进行不同的处理。源处理以下类型的数据：
...
配置REST服务源以处理REST API请求，并将响应传递回源REST API客户端。将源用作[微服务管道的](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Microservice/Microservice_Title.html#concept_qfh_xdm_p2b)一部分。
...
2. 在“ **HTTP”**选项卡上，配置以下属性：
   ...
   | HTTP属性           | 描述                                                         |
   | :----------------- | :----------------------------------------------------------- |
   | HTTP侦听端口       | REST服务来源的侦听端口。端口号必须包含在REST API客户端用于传递数据的URL中。**注意：**没有其他管道或进程已经可以绑定到侦听端口。侦听端口只能由单个管道使用。有关更多信息，请参见[将数据发送到侦听端口](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Origins/RESTService.html#concept_xxr_ftd_kz)。 |
   | 并发请求数上限     | 一次允许将消息发送到源的最大REST API客户端数。如果源达到配置的最大值并接收到来自其他客户端的其他请求，则当插槽可用时，它将处理这些请求。此属性还确定源生成并用于多线程处理的线程数。有关更多信息，请参见[多线程处理](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Origins/RESTService.html#concept_llf_chp_qy)。 |
   | 申请编号           | 用来将请求传递到REST服务源的应用程序ID。应用程序ID必须包含在请求的标头或REST API客户端用来传递数据的URL的查询参数中。有关更多信息，请参见 [在请求中包括应用程序ID](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Origins/RESTService.html#concept_ttw_wtd_kz)。 |
   | URL中的应用程序ID  | 允许从URL读取应用程序ID。当REST API客户端在URL查询参数中而不是请求标头中包含应用程序ID时使用。有关更多信息，请参见[在请求中包括应用程序ID](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Origins/RESTService.html#concept_ttw_wtd_kz)。 |
...
3. 在“ **数据格式”**选项卡上，配置以下属性：
   ...
   | 数据格式属性                                                 | 描述                                                         |
   | :----------------------------------------------------------- | :----------------------------------------------------------- |
   | [资料格式](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Origins/RESTService.html#concept_aaf_ss4_qy) | 要处理的数据类型。使用以下选项之一：阿夫罗二元数据报定界JSON格式原虫[SDC记录](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Pipeline_Design/SDCRecordFormat.html#concept_qkk_mwk_br)XML格式 |
...
12. 在“ **HTTP响应”**选项卡上，配置以下属性：

    | [HTTP响应属性](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Origins/RESTService.html#concept_mhb_sbv_s2b) | 描述                                                         |
    | :----------------------------------------------------------- | :----------------------------------------------------------- |

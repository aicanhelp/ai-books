...
[支持的管道类型：](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Pipeline_Configuration/ProductIcons_Doc.html#concept_mjg_ly5_pgb)![img](imgs/icon-SDC-20200310172846197.png) 资料收集器

MapR Streams使用者来源从MapR Streams读取消息。

配置MapR Streams使用者时，可以配置主题，使用者组和其他常规属性。您可以配置数据类型和相关属性，还可以选择添加其他MapR Streams属性和受支持的Kafka属性。
...
首次启动管道时，MapR Streams使用者将成为该主题的新使用者组。它仅读取传入数据，处理所有分区中的数据，并且默认情况下忽略主题中的任何现有数据。

要读取该主题中的所有未读数据，请添加 **auto.offset.reset** Kafka配置属性并将其设置为 **最早**。有关此属性的更多信息，请参见MapR Streams文档。

## 资料格式

MapR Streams消费者根据数据格式对数据的处理不同。MapR Streams消费者可以处理以下类型的数据：
...
您可以将自定义配置属性添加到MapR Streams Consumer。您可以使用MapR Streams支持的任何MapR或Kafka属性。有关更多信息，请参见MapR Streams文档。

您可以添加任何有效的配置属性。添加属性时，输入确切的属性名称和值。MapR Streams使用者不验证属性名称或值。

## 记录标题属性

MapR Streams消费者来源 会创建记录标题属性，其中包括有关记录的原始文件的信息。当原始处理Avro数据时，它将在AvroSchema记录头属性中包含Avro架构。
...
配置MapR Streams使用者来源以从MapR Streams中读取消息。
...
2. 在“ **MapR流”**选项卡上，配置以下属性：
   ...
   | [MapR流配置](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Origins/MapRStreamsCons.html#concept_lzy_xlg_2v) | 要使用的其他配置属性。使用[简单或批量编辑模式](https://streamsets.com/documentation/controlhub/latest/help/datacollector/UserGuide/Pipeline_Configuration/SimpleBulkEdit.html#concept_alb_b3y_cbb)，单击 **添加**图标以添加属性。使用MapR Streams期望的属性名称和值。您可以使用MapR Streams属性和MapR Streams支持的Kafka属性集。 |
